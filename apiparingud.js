...
async function listiraamatud() {
	    ...
	    try {
        const responseData = await getDataAsJson("https://raamatud-konteiner-dwbba2erd0f6cweq.northeurope-01.azurewebsites.net/raamatud/");
        const resultElement = document.getElementById("raamatud_result");
        resultElement.innerHTML = "";
        for (var raamat of responseData.raamatud) {
            resultElement.innerHTML +=
                '<a href="https://raamatud-konteiner-dwbba2erd0f6cweq.northeurope-01.azurewebsites.net/raamatud/' + raamat + '" download="' + raamat + '.txt">' + raamat + ".txt</a> " +
                '<a href="#" onclick="deleteObject(\'https://raamatud-konteiner-dwbba2erd0f6cweq.northeurope-01.azurewebsites.net/raamatud/' + raamat + '\'); listiraamatud(); return false;"> [kustuta]</a>' +
                "<br />";
        }
    } catch (error) {
        console.error("Raamatute laadimine ebaõnnestus:", error);
    }
}
